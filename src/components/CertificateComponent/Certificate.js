...
export const Certificate = (props) => {
    ...
    const totalCerts = numTotalCertificates;
    const pageSize = 5; // number of certificates to fetch per page

    const fetchMoreData = () => { // for infinite scroll
        concatSomeCertificates(certPage + 1, pageSize);
        setCertPage(certPage + 1);
    }
